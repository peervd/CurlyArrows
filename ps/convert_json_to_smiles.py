...
from .molecular_structures import standardize_molecule_mapping, getSubmolRadAtom, getSubmolRadBond, getRadBond, safe_smiles
...
def parse_json_to_smiles(json_string, mech_category='model'):
    ...
    for i,x in enumerate(synthetic_arrows):
        ...
        if synthetic_arrows[i+1][0] < x[0] < synthetic_arrows[i+1][1] or synthetic_arrows[i+1][0] < x[1] < synthetic_arrows[i+1][1]:
            if x[2] * 1.5 < synthetic_arrows[i+1][2] or x[2] * 1.5 > synthetic_arrows[i+1][2]:
                double = True
    ...
    for mol_number, mol_data in enumerate(molecules):
        ...
        # standardize the molecule mapping
        standardized_mol, std_atom_map, std_atom_ids, std_bond_ids, mapping_dict = standardize_molecule_mapping(
            mol, atom_map[mol_number], atom_ids[mol_number], bond_ids[mol_number]
        )
        
        # Replace the original mappings with standardized ones
        atom_map[mol_number] = std_atom_map
        atom_ids[mol_number] = std_atom_ids
        bond_ids[mol_number] = std_bond_ids
        m[mol_number] = standardized_mol if standardized_mol is not None else mol
                
        smiles = Chem.MolToSmiles(m[mol_number])
        ...
        for x in coordinates:
            ...
            x_max =  x[1][1]
            y =      x[1][2]
            avg_arrow_y = sum([arrow[2] for arrow in synthetic_arrows]) / len(synthetic_arrows) if synthetic_arrows else y
            
            if len(x[0]) > 1:
                if x_min < avg_x < x_max and avg_y < (avg_arrow_y + 50):
                    category = x[0][0]
                elif x_min < avg_x < x_max and avg_y > (avg_arrow_y + 50):
                    category = x[0][1]
    ...
    for arrow in curved_arrows:
        ...
        if s == 'atom':
            ...
            elif len_start > 2 and start_arrow != 'H':
                SubMol['start_sub'] = getSubmolRadAtom(m[mol_start], start_a, 2, True)
            elif len_start > 2:
                SubMol['start_sub'] = getSubmolRadAtom(m[mol_start], start_a, 2, False)
                sanitize = Chem.MolToSmiles(m[mol_start])
        elif s == 'bond':
            if len_start <= 2:
                ...
                if 'H' not in start_arrow:
                    SubMol['start_sub'] = getSubmolRadBond(m[mol_start], start_a, 2, True)
                else:
                    SubMol['start_sub'] = getSubmolRadBond(m[mol_start], start_a, 2, False)
                    sanitize = Chem.MolToSmiles(m[mol_start])
        ...
        if e == 'atom':
            ...
            elif len_end > 2 and end_arrow != 'H':
                SubMol['end_sub'] = getSubmolRadAtom(m[mol_end], end_a, 2, True)
            elif len_end > 2:
                SubMol['end_sub'] = getSubmolRadAtom(m[mol_end], end_a, 2, False)
                sanitize = Chem.MolToSmiles(m[mol_end])
        elif e == 'bond':
            if len_start <= 2:
                ...
                if 'H' not in end_arrow:
                    SubMol['end_sub'] = getSubmolRadBond(m[mol_end], end_a, 2, True)
                else:
                    SubMol['end_sub'] = getSubmolRadBond(m[mol_end], end_a, 2, False)
                    sanitize = Chem.MolToSmiles(m[mol_end])
        ...
        for key in smiles_dict:
            if mol_start in smiles_dict[key]['molecules'].keys():
                set_key = key
                if sanitize:
                    skip_sanitize_H.append((set_key,sanitize))
                break

        if set_key is not None and start is not None and end is not None:
            smiles_dict[set_key]["arrows"].append({
                "start_molecule": safe_smiles(Chem.MolToSmiles(m[mol_start])) if 'H' not in start_arrow else Chem.MolToSmiles(m[mol_start]),
                "start_arrow": start_arrow,
                "substructure_start": SubMol['start_sub'], 
                "end_molecule": safe_smiles(Chem.MolToSmiles(m[mol_end])) if 'H' not in end_arrow else Chem.MolToSmiles(m[mol_end]),
                "end_arrow": end_arrow,
                "substructure_end": SubMol['end_sub'],
                "electrons": arrow.get("e")
            })
    
    # Remove irrelevant H from molecules
    for step in smiles_dict.keys():
        for molecule in smiles_dict[step]['molecules']:
            if (step,smiles_dict[step]['molecules'][molecule]) in skip_sanitize_H:
                pass 
            else:
                san_mol = smiles_dict[step]['molecules'][molecule]
                smiles_dict[step]['molecules'][molecule] = safe_smiles(san_mol)
